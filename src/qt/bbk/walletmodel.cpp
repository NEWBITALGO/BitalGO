...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 ALG.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzALGReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzALGReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzALGReset.connect(boost::bind(NotifyzALGReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzALGReset.disconnect(boost::bind(NotifyzALGReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zALGTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
