...
#include "ui_zALGcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zALGTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zALGTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zALG.";
            else if (!mint.isSeedCorrect)
                strReason = "The zALG seed used to mint this zALG is not the same as currently hold in the wallet";
            else
