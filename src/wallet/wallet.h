...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zALGspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ALG at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zALG send
enum ZerocoinSpendStatus {
    ZALG_SPEND_OKAY = 0,                            // No error
    ZALG_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZALG_WALLET_LOCKED = 2,                         // Wallet was locked
    ZALG_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZALG_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZALG_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZALG_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZALG_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZALG_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZALG_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZALG_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZALG_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZALG_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZALG_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZALG_SPENT_USED_ZALG = 14,                      // Coin has already been spend
    ZALG_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZALG_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZALGOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzALGAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzALGTracker> zALGTracker;
    ...
    void setZWallet(CzALGWallet* zwallet)
    {
        zwalletMain = zwallet;
        zALGTracker = std::unique_ptr<CzALGTracker>(new CzALGTracker(strWalletFile));
    }

    CzALGWallet* getZWallet() { return zwalletMain; }
    ...
    /** zALG reset */
    boost::signals2::signal<void()> NotifyzALGReset;
