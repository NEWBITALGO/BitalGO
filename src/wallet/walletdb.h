...
// Copyright (c) 2016-2019 The BitalGo developers
...
public:
    ...
    bool WriteZALGSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZALGSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZALGSeed_deprecated(uint256& seed);
    bool EraseZALGSeed();
    bool EraseZALGSeed_deprecated();

    bool WriteZALGCount(const uint32_t& nCount);
    bool ReadZALGCount(uint32_t& nCount);
