...
CzALGWallet::CzALGWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        ...
        if (walletdb.ReadZALGSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZALGSeed_deprecated()) {
...
bool CzALGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZALGCount(nCountLastUsed);
    else if (!walletdb.ReadZALGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzALGWallet::Lock()
{
    seedMaster = 0;
}

void CzALGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzALGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZALG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzALGWallet::LoadMintPoolFromDB()
{
...
void CzALGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzALGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzALGWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzALGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZALG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzALGWallet::SeedToZALG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzALGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzALGWallet::GenerateDeterministicZALG(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzALGWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzALGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzALGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
