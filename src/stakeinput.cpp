...
// The zALG block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zALG is a hash of the serial
...
{
    //Create an output returning the zALG that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZALGOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zALG output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zALG", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZALGOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zALG output", __func__);
...
{
    CzALGTracker* zALGTracker = pwallet->zALGTracker.get();
    CMintMeta meta;
    if (!zALGTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zALGTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
